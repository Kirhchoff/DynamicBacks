function createDynamicBack() {
  ...
  ._dynamic_back{0}:after{background-image:url({2});background-size:cover;z-index:-2;}";
  ...
  function _createDynamicBackCSS(duration, color){
    var numOfBacks = _backs.length;
    $("<style type='text/css'>"+_formatString(_dynamicBackBaseTmpl, color)+"</style>").appendTo("head");
    for(var i = 0; i<numOfBacks; i++){
      $("<style type='text/css'>"+_formatString(_dynamicBackTmpl,i,_backs[i], _backs[(i+1)%numOfBacks])+"</style>").appendTo("head");
    }
